...
UserPar.add("InPlaneRes", 10)    # Choose odd number
UserPar.add("OutPlaneRes", 10)   # Choose odd number
UserPar.add("chi", 0.3)
...
# Stabilized step
StStep1 = np.linspace(1E6, 1E5, TotSurfSteps + TotEqm1)
StStep2 = np.linspace(5E2, 1E4, TotChemSteps + TotEqm2)
StStep = np.concatenate([StStep1, StStep2])
StExp = Expression("St", St=1E4, degree=0)
...
SimPar4 = "_l0_%.1e" % (l0)
if St == 1:
    SaveDir = "../Results/CubeFreeS/" + SimPar1 + SimPar2 + SimPar3 + SimPar4 + "/"
else:
    SaveDir = "../Results/CubeFree/" + SimPar1 + SimPar2 + SimPar3 + SimPar4 + "/"
...
if St == 1:
    ...
    V = FunctionSpace(mesh, V_CG1elem*CG1elem)
else:
...
for (Step, Surf) in enumerate(SurfSteps):
    ...
    if MPI.rank(MPI.comm_world) == 0:
        print("\033[1;32m--- Step {0:2d} at time {1:.0e}: Surf = {2:.3f}, Chem = {3:.2e} ---\033[1;m".format(Step, t, Surf, ChemSteps[Step]))
