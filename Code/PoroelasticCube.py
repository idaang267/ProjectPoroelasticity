...
import matplotlib.pyplot as plt         # Module matplotlib for ploT_DG0ing
import numpy as np
import time
...
# Lateral surfaces
class SymX(SubDomain):    # left and right surfaces
    def inside(self, x, on_boundary):
        return near(x[0], -.5) or near(x[0], 0.5) and on_boundary
class SymZ(SubDomain):    # front and back surfaces
    def inside(self, x, on_boundary):
        return near(x[2], -.5) or near(x[2], 0.5) and on_boundary
...
UserPar.add("InPlaneRes", 8)
UserPar.add("OutPlaneRes", 8)
UserPar.add("chi", 0.3)
UserPar.add("GMax", 0.01)
UserPar.add("l0", 2.)
UserPar.add("IniTime", 10**(-3))
UserPar.add("MidTime", 10**(-1))
UserPar.add("FinTime", 10**(2))
UserPar.add("TotSurfSteps", 50)
UserPar.add("TotEqm1", 50)
UserPar.add("TotChemSteps", 10)
UserPar.add("TotEqm2", 50)
UserPar.parse()

# Parse from command line if given
tol = 1E-12
InPlaneRes = UserPar["InPlaneRes"]      # Along top surface (without refinement)
OutPlaneRes = UserPar["OutPlaneRes"]
...
SurfSteCG1 = np.linspace(0, GMax ,TotSurfSteps)
SurfSteV_CG2 = np.linspace(GMax, GMax, TotEqm1+TotChemSteps+TotEqm2)
SurfSteps = np.concatenate([SurfSteCG1, SurfSteV_CG2])
...
SimPar3 = "_G_%.0e" % (GMax)
SimPar4 = "_l0_%.1e" % (l0)
SaveDir = "../Results/CubeBound/" + SimPar1 + SimPar2 + SimPar3 + SimPar4 + "/"
...
# Create SubDomains, Boundaries, and Points
...
SymX.mark(Boundaries, 1)
SymZ.mark(Boundaries, 2)
TopBound.mark(Boundaries, 3)
BotBound.mark(Boundaries, 4)
File = XDMFFile(SaveDir + "/" + "Boundaries.xdmf")
...
uBot = Expression(("(l0-1)*x[0]", "0.0*x[1]", "(l0-1)*x[2]"), l0=l0, degree=1)
uSymX = Expression(("(l0-1)*x[0]"), l0=l0, degree=1)
uSymZ = Expression(("(l0-1)*x[2]"), l0=l0, degree=1)

# The Dirichlet BCs are specified in respective subspaces
DispBCBot = DirichletBC(V.sub(0), uBot, BotBound)
# Chemical boundary conditions
ChemBCTop = DirichletBC(V.sub(1), ChemBC, TopBound)
ChemBCSymX = DirichletBC(V.sub(1), ChemBC, SymX)
ChemBCSymZ = DirichletBC(V.sub(1), ChemBC, SymZ)

# Combined boundary conditions
bc = [DispBCBot, ChemBCTop, ChemBCSymX, ChemBCSymZ]
...
for (Step, Surf) in enumerate(SurfSteps):
    ...
    if MPI.rank(MPI.comm_world) == 0:
        print("\033[1;32m--- Step {0:2d} at time {1:.2f}: Surf = {2:.3f}, Chem = {3:.3f} ---\033[1;m".format(Step, t, Surf, ChemSteps[Step]))
    ...
    # Postprocessing
    #---------------------------------------------------------------------------
    PostProc[Step, :] = np.array([Step, TimeSteps[Step], t, Surf, ChemSteps[Step], u(0,0,0.5)[0]])
